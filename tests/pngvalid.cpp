...
#define PNG_GAMMA_THRESHOLD_FIXED ((png_fixed_point)(PNG_GAMMA_THRESHOLD * PNG_FP_1))
#endif
 ...
 */
#define anon_context(ps) struct exception_context* the_exception_context = &(ps)->exception_context
...
static size_t standard_name (char* buffer, size_t bufsize, size_t pos, png_byte colour_type,
...
static size_t standard_name_from_id (char* buffer, size_t bufsize, size_t pos, png_uint_32 id)
{
  return standard_name (buffer, bufsize, pos, COL_FROM_ID (id), DEPTH_FROM_ID (id),
                        PALETTE_FROM_ID (id), INTERLACE_FROM_ID (id), WIDTH_FROM_ID (id),
...
typedef struct store_pool
{
  ...
  size_t max;     /* Maximum single allocation */
  size_t current; /* Current allocation */
  size_t limit;   /* Highest current allocation */
  size_t total;   /* Total allocation */
...
static size_t store_message (png_store* ps, const png_struct* pp, char* buffer, size_t bufsize,
...
static void store_verbose (png_store* ps, const png_struct* pp, png_const_charp prefix,
...
static void store_log (png_store* ps, const png_struct* pp, png_const_charp message, int is_error)
{
  /* The warning is copied to the error buffer if there are no errors and it is
   * the first warning.  The error is copied to the error buffer if it is the
   * first error (overwriting any prior warnings).
   */
  if (is_error ? (ps->nerrors)++ == 0 : (ps->nwarnings)++ == 0 && ps->nerrors == 0)
...
static png_bytep store_image_row (const png_store* ps, const png_struct* pp, int nImage,
                                  png_uint_32 y)
{
...
static void store_image_free (png_store* ps, const png_struct* pp)
{
  if (ps->image != NULL)
  {
...
static void store_ensure_image (png_store* ps, const png_struct* pp, int nImages, size_t cbRow,
                                png_uint_32 cRows)
{
...
static void store_image_check (const png_store* ps, const png_struct* pp, int iImage)
{
  ...
  else
  {
    ...
    for (; rows > 0; --rows)
    {
      ...
      if (image[cbRow] != 222 || image[cbRow + 1] != 173 || image[cbRow + 2] != 17)
...
static void PNGCBAPI store_flush (png_struct* ppIn)
{
    ...
    do
    {
      ...
      else /* !IDAT */
      {
        ...
        else /* Return chunk bytes, including the CRC */
        {
           ...
           */
          if (chunkpos >= chunklen && max - st >= min && store_read_buffer_avail (ps) == 0)
            break;
        }
      } /* !IDAT */
    } while (st > 0);
...
static void store_progressive_read (png_store* ps, png_struct* pp, png_infop pi)
{
...
static void PNGCBAPI store_free (png_struct* ppIn, png_voidp memory)
{
   ...
   */
  --store;
  for (test = &pool->list; *test != store; test = &(*test)->next)
  {
    if (*test == NULL)
    {
 ...
 */
static png_struct* set_store_for_write (png_store* ps, png_infopp ppi, const char* name)
{
  try
  {
    if (ps->pwrite != NULL)
 ...
 */
static png_struct* set_store_for_read (png_store* ps, png_infopp ppi, png_uint_32 id,
                                       const char* name)
{
...
{
  return !pm->log && !pm->verbose
         && (pm->nerrors > 0 || (pm->treat_warnings_as_errors && pm->nwarnings > 0));
}
 ...
 * 257!
 */
static int output_quantization_factor (const png_modifier* pm, int in_depth, int out_depth)
{
...
{
  pos = safecat_color_encoding (buffer, bufsize, pos, pm->current_encoding, pm->current_gamma);
...
static void modifier_read_imp (png_modifier* pm, png_bytep pb, size_t st)
{
  while (st > 0)
  {
    ...
    if (pm->buffer_position >= pm->buffer_count)
    {
      ...
      {
      case modifier_start:
        store_read_chunk (pm, pm->buffer, 8, 8); /* signature. */
        pm->buffer_count = 8;
        pm->buffer_position = 0;

        if (memcmp (pm->buffer, sign, 8) != 0)
          png_error (pm->pread, "invalid PNG file signature");
        pm->state = modifier_signature;
        break;

      case modifier_signature:
        store_read_chunk (pm, pm->buffer, 13 + 12, 13 + 12); /* IHDR */
        pm->buffer_count = 13 + 12;
        pm->buffer_position = 0;

        if (png_get_uint_32 (pm->buffer) != 13 || png_get_uint_32 (pm->buffer + 4) != CHUNK_IHDR)
          png_error (pm->pread, "invalid IHDR");

        /* Check the list of modifiers for modifications to the IHDR. */
        mod = pm->modifications;
        while (mod != NULL)
        {
          if (mod->chunk == CHUNK_IHDR && mod->modify_fn && (*mod->modify_fn) (pm, mod, 0))
          {
            mod->modified = 1;
      ...
      default:
        ...
        if (pm->pending_chunk != 0)
        {
          ...
          store_read_chunk (pm, pm->buffer, 8, 8);
        ...
        if (chunk == CHUNK_PLTE || chunk == CHUNK_IDAT || chunk == CHUNK_IEND)
        {
          ...
          while (mod != NULL)
          {
            if ((mod->add == chunk || (mod->add == CHUNK_PLTE && chunk == CHUNK_IDAT))
        ...
        if (len + 12 <= sizeof pm->buffer)
        {
          ...
          store_read_chunk (pm, pm->buffer + pm->buffer_count, s, s);
 ...
 * need a local buffer.
 */
static void modifier_progressive_read (png_modifier* pm, png_struct* pp, png_infop pi)
{
  if (pm->pread != pp || pm->current == NULL || pm->next == NULL)
    png_error (pp, "store state damaged (progressive)");
  ...
  for (;;)
  {
     ...
     */
    cbAvail = store_read_buffer_avail (pm);
   ...
   * file!)
   */
  if (pm->buffer_count > pm->buffer_position || pm->next != &pm->current->data
      || pm->readpos < pm->current->datacount)
    png_error (pp, "progressive read implementation error");
}

/* Set up a modifier. */
static png_struct* set_modifier_for_read (png_modifier* pm, png_infopp ppi, png_uint_32 id,
                                          const char* name)
{
  ...
  return set_store_for_read (pm, ppi, id, name);
}
 ...
 */
struct gama_modification : public png_modification
{
...
static void gama_modification_init (gama_modification* me, png_modifier* pm, double gammad)
{
  double g;

  modification_init (me);
  me->chunk = CHUNK_gAMA;
  me->modify_fn = gama_modify;
  me->add = CHUNK_PLTE;
  g = fix (gammad);
...
struct chrm_modification : public png_modification
{
                                    ...
                                    const color_encoding* encoding)
{
  ...
  modification_init (me);
  me->chunk = CHUNK_cHRM;
  me->modify_fn = chrm_modify;
  me->add = CHUNK_PLTE;
  me->next = pm->modifications;
  pm->modifications = me;
}

struct srgb_modification : png_modification
{
  png_byte intent;
};
...
static void srgb_modification_init (srgb_modification* me, png_modifier* pm, png_byte intent)
{
  modification_init (me);
  me->chunk = CHUNK_sBIT;
...
static int sbit_modify (png_modifier* pm, png_modification* me, int add)
{
  ...
  if (pm->bit_depth > sbit)
  {
    ...
    default:
      png_error (pm->pread, "unexpected colour type in sBIT modification");
    }
...
static void sbit_modification_init (sbit_modification* me, png_modifier* pm, png_byte sbit)
{
  modification_init (me);
  me->chunk = CHUNK_sBIT;
  me->modify_fn = sbit_modify;
  me->add = CHUNK_PLTE;
  me->sbit = sbit;
  me->next = pm->modifications;
  pm->modifications = me;
...
static void init_standard_palette (png_store* ps, png_struct* pp, png_infop pi, int npalette,
                                   int do_tRNS)
{
 ...
 */
static int npasses_from_interlace_type (const png_struct* pp, int interlace_type)
{
...
static unsigned int bit_size (const png_struct* pp, png_byte colour_type, png_byte bit_depth)
{
  switch (colour_type)
  {
  default:
...
static size_t transform_rowsize (const png_struct* pp, png_byte colour_type, png_byte bit_depth)
{
...
static png_uint_32 transform_height (const png_struct* pp, png_byte colour_type, png_byte bit_depth)
{
 ...
 */
static png_uint_32 standard_width (const png_struct* pp, png_uint_32 id)
{
...
static png_uint_32 standard_height (const png_struct* pp, png_uint_32 id)
{
...
static png_uint_32 standard_rowsize (const png_struct* pp, png_uint_32 id)
{
...
#define TEXT_COMPRESSION PNG_TEXT_COMPRESSION_NONE
    ...
    if (png_get_rowbytes (pp, pi) != transform_rowsize (pp, colour_type, bit_depth))
      ...
      for (pass = 0; pass < npasses; ++pass)
      {
        ...
        for (y = 0; y < h; ++y)
        {
          ...
          if (interlace_type == PNG_INTERLACE_ADAM7)
          {
             ...
             */
            if (PNG_ROW_IN_INTERLACE_PASS (y, pass) && PNG_PASS_COLS (w, pass) > 0)
              interlace_row (buffer, buffer, bit_size (pp, colour_type, bit_depth), w, pass,
    ...
    store_storefile (ps, FILEID (colour_type, bit_depth, palette_number, interlace_type, 0, 0, 0));

    store_write_reset (ps);
  }

  catch (png_store *fault)
  {
...
static void make_transform_images (png_modifier* pm)
{
  ...
  safecat (pm->test, sizeof pm->test, 0, "make standard images");
  ...
  while (next_format (&colour_type, &bit_depth, &palette_number, 1, 1))
  {
    ...
    for (interlace_type = PNG_INTERLACE_NONE; interlace_type < INTERLACE_LAST; ++interlace_type)
    {
      ...
      standard_name (name, sizeof name, 0, colour_type, bit_depth, palette_number, interlace_type,
                     0, 0, do_own_interlace);
      make_transform_image (pm, colour_type, bit_depth, palette_number, interlace_type,
                            name);
    }
  }
}
 ...
 */
static void size_row (png_byte buffer[SIZE_ROWMAX], png_uint_32 bit_width, png_uint_32 y)
{
...
static void make_size_image (png_store* const ps, png_byte const colour_type,
                             ...
                             png_uint_32 const w, png_uint_32 const h, int const do_interlace)
{
  check_interlace_type (interlace_type);
  ...
  try
  {
    ...
    char name[FILE_NAME_SIZE];
    png_uint_32 id =
      FILEID (colour_type, bit_depth, 0 /*palette*/, interlace_type, w, h, do_interlace);
    ...
    else
    {
      ...
      if (!do_interlace && npasses != set_write_interlace_handling (pp, interlace_type))
        png_error (pp, "write: png_set_interlace_handling failed");
        ...
        for (y = 0; y < h; ++y)
        {
          ...
          if (do_interlace && interlace_type == PNG_INTERLACE_ADAM7)
          {
            ...
            if (PNG_ROW_IN_INTERLACE_PASS (y, pass) && wPass > 0)
            {
              ...
              interlace_row (tempRow, row, pixel_size, w, pass, 0 /*data always bigendian*/);
              row = tempRow;
            }
            else
              continue;
...
static void make_size (png_store* const ps, png_byte const colour_type, int bdlo, int const bdhi)
{
  for (; bdlo <= bdhi; ++bdlo)
  {
    png_uint_32 width;
    ...
    for (width = 1; width <= 16; ++width)
    {
      ...
      for (height = 1; height <= 16; ++height)
      {
         ...
         */
        make_size_image (ps, colour_type, DEPTH (bdlo), PNG_INTERLACE_ADAM7, width, height, 1);
#endif
      }
    }
  }
...
static void sBIT_error_fn (png_struct* pp, png_infop pi)
{
  png_byte bit_depth;
  png_color_8 bad;
  ...
  bad.red = bad.green = bad.blue = bad.gray = bad.alpha = (png_byte)(bit_depth + 1);
  png_set_sBIT (pp, pi, &bad);
}

static const struct
{
  void (*fn) (png_struct*, png_infop);
  const char* msg;
  unsigned int warning : 1; /* the error is a warning... */
} error_test[] = {
  /* no warnings makes these errors undetectable prior to 1.7.0 */
  {sBIT0_error_fn, "sBIT(0): failed to detect error", PNG_LIBPNG_VER < 10700},

  {sBIT_error_fn, "sBIT(too big): failed to detect error", PNG_LIBPNG_VER < 10700},
};

static void make_error (png_store* const ps, png_byte const colour_type, png_byte bit_depth,
...
static int make_errors (png_modifier* const pm, png_byte const colour_type, int bdlo,
                        int const bdhi)
{
  for (; bdlo <= bdhi; ++bdlo)
  {
    ...
    for (interlace_type = PNG_INTERLACE_NONE; interlace_type < INTERLACE_LAST; ++interlace_type)
    {
      ...
      standard_name (name, sizeof name, 0, colour_type, 1 << bdlo, 0, interlace_type, 0, 0,
                     do_own_interlace);

      for (test = 0; test < ARRAY_SIZE (error_test); ++test)
      {
        make_error (pm, colour_type, DEPTH (bdlo), interlace_type, test, name);
                                   ...
                                   int do_interlace, int use_update_info)
{
  ...
  if (dp->colour_type == 3)
    ...
    dp->red_sBIT = dp->blue_sBIT = dp->green_sBIT = dp->alpha_sBIT = dp->bit_depth;
 ...
 */
static int read_palette (store_palette palette, int* npalette, const png_struct* pp, png_infop pi)
{
  png_colorp pal;
 ...
 * non-transform case).
 */
static void standard_palette_validate (standard_display* dp, const png_struct* pp, png_infop pi)
{
  int npalette;
  store_palette palette;
...

  {
    ...
    while (--i >= 0)
      if (palette[i].red != dp->palette[i].red || palette[i].green != dp->palette[i].green
          || palette[i].blue != dp->palette[i].blue || palette[i].alpha != dp->palette[i].alpha)
 ...
 */
static void standard_info_part1 (standard_display* dp, png_struct* pp, png_infop pi)
{
   ...
   */
  {
    ...
    if (png_get_tRNS (pp, pi, 0, 0, &trans_color) & PNG_INFO_tRNS)
    {
      ...
      {
      case 0:
        dp->transparent.red = dp->transparent.green = dp->transparent.blue = trans_color->gray;
        dp->has_tRNS = 1;
        break;
 ...
 * image.
 */
static void standard_info_part2 (standard_display* dp, const png_struct* pp, png_const_infop pi,
                                 int nImages)
{
  /* Record cbRow now that it can be found. */
  {
    ...
    if (bd >= 8 && (ct == PNG_COLOR_TYPE_RGB || ct == PNG_COLOR_TYPE_GRAY) && dp->filler)
...
static void standard_info_imp (standard_display* dp, png_struct* pp, png_infop pi, int nImages)
{
...
static void sequential_row (standard_display* dp, png_struct* pp, png_infop pi, int iImage,
                            int iDisplay)
{
  int npasses = dp->npasses;
  int do_interlace = dp->do_interlace && dp->interlace_type == PNG_INTERLACE_ADAM7;
    ...
    for (y = 0; y < height; ++y)
    {
      if (do_interlace)
      {
        ...
        if (wPass > 0 && PNG_ROW_IN_INTERLACE_PASS (y, pass))
        {
          ...
          if (iImage >= 0)
            deinterlace_row (store_image_row (ps, pp, iImage, y), row, dp->pixel_size, dp->w, pass,
                             dp->littleendian);

          if (iDisplay >= 0)
            deinterlace_row (store_image_row (ps, pp, iDisplay, y), display, dp->pixel_size, dp->w,
                             pass, dp->littleendian);
        }
      }
      else
        png_read_row (pp, iImage >= 0 ? store_image_row (ps, pp, iImage, y) : NULL,
                                 ...
                                 png_const_charp text)
{
  ...
  else
  {
    ...
    {
      char buf[64];
      sprintf (buf, "text length changed[%lu->%lu], ", (unsigned long)strlen (text),
...
static void standard_text_validate (standard_display* dp, const png_struct* pp, png_infop pi,
                                    int check_end)
{
  png_textp tp = NULL;
                                   ...
                                   int iDisplay, png_uint_32 y)
{
   ...
   */
  if (iImage >= 0
      && (where = pixel_cmp (std, store_image_row (dp->ps, pp, iImage, y), dp->bit_width)) != 0)
  {
    char msg[64];
    sprintf (msg, "PNG image row[%lu][%d] changed from %.2x to %.2x", (unsigned long)y, where - 1,
             std[where - 1], store_image_row (dp->ps, pp, iImage, y)[where - 1]);
    png_error (pp, msg);
  }

  if (iDisplay >= 0
      && (where = pixel_cmp (std, store_image_row (dp->ps, pp, iDisplay, y), dp->bit_width)) != 0)
  {
    char msg[64];
    sprintf (msg, "display row[%lu][%d] changed from %.2x to %.2x", (unsigned long)y, where - 1,
             std[where - 1], store_image_row (dp->ps, pp, iDisplay, y)[where - 1]);
    png_error (pp, msg);
  }
}

static void standard_image_validate (standard_display* dp, const png_struct* pp, int iImage,
                                     int iDisplay)
{
  png_uint_32 y;
...
static void PNGCBAPI standard_end (png_struct* ppIn, png_infop pi)
{
   ...
   * interlaced images.
   */
  standard_text_validate (dp, pp, pi, PNG_LIBPNG_VER >= 10518 /*check_end: see comments above*/);
  standard_image_validate (dp, pp, 0, -1);
}

/* A single test run checking the standard image to ensure it is not damaged. */
static void standard_test (png_store* const psIn, png_uint_32 const id, int do_interlace,
                           int use_update_info)
{
  standard_display d;
  ...
  try
  {
    ...
    if (d.ps->progressive)
    {
      png_set_progressive_read_fn (pp, &d, standard_info, progressive_row, standard_end);
...
static int test_standard (png_modifier* const pm, png_byte const colour_type, int bdlo,
                          int const bdhi)
{
  for (; bdlo <= bdhi; ++bdlo)
  {
    ...
    {
      standard_test (pm,
                     FILEID (colour_type, DEPTH (bdlo), 0 /*palette*/, interlace_type, 0, 0, 0),
...
/********************************** SIZE TESTS ********************************/
static int test_size (png_modifier* const pm, png_byte const colour_type, int bdlo, int const bdhi)
{
  ...
  for (; bdlo <= bdhi; ++bdlo)
  {
    ...
    for (h = 1; h <= 16; h += hinc[bdlo])
    {
      for (w = 1; w <= 16; w += winc[bdlo])
      {
         ...
         */
        standard_test (
          pm, FILEID (colour_type, DEPTH (bdlo), 0 /*palette*/, PNG_INTERLACE_NONE, w, h, 0),
        ...
        standard_test (
          pm, FILEID (colour_type, DEPTH (bdlo), 0 /*palette*/, PNG_INTERLACE_NONE, w, h, 1),
          0 /*do_interlace*/, pm->use_update_info);
        ...
        standard_test (
          pm,
          FILEID (colour_type, DEPTH (bdlo), 0 /*palette*/, PNG_INTERLACE_ADAM7, w, h, 1),
          1 /*do_interlace*/, pm->use_update_info);
  ...
  for (bdlo = save_bdlo; bdlo <= bdhi; ++bdlo)
  {
    ...
    for (h = 1; h <= 16; h += hinc[bdlo])
    {
      for (w = 1; w <= 16; w += winc[bdlo])
      {
        ...
        standard_test (
          pm,
          FILEID (colour_type, DEPTH (bdlo), 0 /*palette*/, PNG_INTERLACE_ADAM7, w, h, 0),
          1 /*do_interlace*/, pm->use_update_info);
...
static void image_pixel_setf (image_pixel* image, unsigned int rMax, unsigned int gMax,
                              unsigned int bMax, unsigned int aMax)
{
  image->redf = image->red / (double)rMax;
  image->greenf = image->green / (double)gMax;
  image->bluef = image->blue / (double)bMax;
  image->alphaf = image->alpha / (double)aMax;

  if (image->red < rMax)
    image->rede = image->redf * DBL_EPSILON;
  else
    image->rede = 0;
  if (image->green < gMax)
    image->greene = image->greenf * DBL_EPSILON;
  else
    image->greene = 0;
  if (image->blue < bMax)
    image->bluee = image->bluef * DBL_EPSILON;
  else
    image->bluee = 0;
  if (image->alpha < aMax)
    image->alphae = image->alphaf * DBL_EPSILON;
  else
    image->alphae = 0;
}
 ...
 * reset.
 */
static void image_pixel_init (image_pixel* image, png_const_bytep row, png_byte colour_type,
                              png_byte bit_depth, png_uint_32 x, store_palette palette,
                              const image_pixel* format /*from pngvalid transform of input*/)
{
  png_byte sample_depth = (png_byte)(colour_type == PNG_COLOR_TYPE_PALETTE ? 8 : bit_depth);
   ...
   * rgb(x,x,x)!
   */
  image->palette_index = image->red = image->green = image->blue =
    sample (row, colour_type, bit_depth, x, 0, swap16, littleendian);
  image->alpha = max;
  image->red_sBIT = image->green_sBIT = image->blue_sBIT = image->alpha_sBIT = sample_depth;
  ...
  if (colour_type == 3) /* palette */
  {
    ...
    if (palette != 0)
    {
      unsigned int i = image->palette_index;

      image->red = palette[i].red;
      image->green = palette[i].green;
      image->blue = palette[i].blue;
      image->alpha = palette[i].alpha;
    }
  }
  ...
  else /* not palette */
  {
    ...
    if (colour_type & 2)
    {
      ...
      image->green = sample (row, colour_type, bit_depth, x, ++i, swap16, littleendian);

      if (format != 0 && format->swap_rgb) /* BGR */
        image->red = sample (row, colour_type, bit_depth, x, ++i, swap16, littleendian);
      else
        image->blue = sample (row, colour_type, bit_depth, x, ++i, swap16, littleendian);
    }

    else /* grayscale */
      if (format != 0 && format->mono_inverted)
        image->red = image->green = image->blue = image->red ^ max;

    if ((colour_type & 4) != 0) /* alpha */
    {
      if (format == 0 || !format->alpha_first)
        image->alpha = sample (row, colour_type, bit_depth, x, ++i, swap16, littleendian);
   ...
   */
  image_pixel_setf (image, sig_bits ? (1U << format->red_sBIT) - 1 : max,
   ...
   */
  image->colour_type = colour_type;
  image->bit_depth = bit_depth;
  image->sample_depth = sample_depth;
  image->have_tRNS = 0;
  image->swap_rgb = 0;
  image->alpha_first = 0;
  image->alpha_inverted = 0;
  image->mono_inverted = 0;
  image->swap16 = 0;
 ...
 */
static void image_pixel_convert_PLTE (image_pixel* image)
{
  if (image->colour_type == PNG_COLOR_TYPE_PALETTE)
  {
    if (image->have_tRNS)
    {
      image->colour_type = PNG_COLOR_TYPE_RGB_ALPHA;
      image->have_tRNS = 0;
    }
    else
      image->colour_type = PNG_COLOR_TYPE_RGB;
 ...
 * records this.
 */
static void image_pixel_add_alpha (image_pixel* image, const standard_display* display,
                                   int for_background)
{
  if (image->colour_type == PNG_COLOR_TYPE_PALETTE)
    image_pixel_convert_PLTE (image);

  if ((image->colour_type & PNG_COLOR_MASK_ALPHA) == 0)
  {
    if (image->colour_type == PNG_COLOR_TYPE_GRAY)
    {
      ...
      if (image->have_tRNS)
      {
         ...
         * tRNS chunk to expand at image point.
         */
#if PNG_LIBPNG_VER >= 10700
        if (!for_background && image->bit_depth < 8)
          image->bit_depth = image->sample_depth = 8;
         ...
         */
        if (image->red == display->transparent.red)
          image->alphaf = 0;
        else
          image->alphaf = 1;
      }
      ...
      image->colour_type = PNG_COLOR_TYPE_GRAY_ALPHA;
    }

    else if (image->colour_type == PNG_COLOR_TYPE_RGB)
    {
      if (image->have_tRNS)
      {
        image->have_tRNS = 0;
         ...
         * value!
         */
        if (image->red == display->transparent.red && image->green == display->transparent.green
            && image->blue == display->transparent.blue)
          image->alphaf = 0;
        else
          image->alphaf = 1;
      }
...
struct image_transform
{
   ...
   */
  void (*ini) (const struct image_transform* in, struct transform_display* out);

  /* And a callback to set the transform on the current png_read_struct:
   */
  void (*set) (const struct image_transform* in, struct transform_display* out, png_struct* pp,
               png_infop pi);
   ...
   */
  void (*mod) (const struct image_transform* in, image_pixel* out, const png_struct* pp,
   ...
   * point running it.
   */
  int (*add) (struct image_transform* in, const struct image_transform** out,
              png_byte colour_type, png_byte bit_depth);
};

struct transform_display : public standard_display
{
  /* Parameters */
  png_modifier* pm;
...
static void transform_set_encoding (transform_display* display)
{
  ...
  if (modifier_color_encoding_is_set (pm))
  {
    if (modifier_color_encoding_is_sRGB (pm))
      srgb_modification_init (&display->srgb_mod, pm, PNG_sRGB_INTENT_ABSOLUTE);

    else
    {
      /* Set gAMA and cHRM separately. */
      gama_modification_init (&display->gama_mod, pm, pm->current_gamma);

      if (pm->current_encoding != 0)
        chrm_modification_init (&display->chrm_mod, pm, pm->current_encoding);
    }
  }
}

/* Three functions to end the list: */
static void image_transform_ini_end (const image_transform*, transform_display*)
{
}

static void image_transform_set_end (const image_transform*, transform_display*,
                                     png_struct* pp, png_infop pi)
{
}
...
static void image_transform_mod_end (const image_transform* in, image_pixel* out,
                                     const png_struct* pp, const transform_display* display)
{
  unsigned int scale = (1U << out->sample_depth) - 1;
  int sig_bits = out->sig_bits;
   ...
   */
  out->red = sample_scale (out->redf, scale);
   ...
   */
  if (sig_bits && out->red_sBIT < out->sample_depth)
    out->red >>= out->sample_depth - out->red_sBIT;
   ...
   */
  out->rede += 1. / (2 * ((1U << out->red_sBIT) - 1));

  if (out->colour_type & PNG_COLOR_MASK_COLOR)
  {
    out->green = sample_scale (out->greenf, scale);
    if (sig_bits && out->green_sBIT < out->sample_depth)
      out->green >>= out->sample_depth - out->green_sBIT;

    out->blue = sample_scale (out->bluef, scale);
    if (sig_bits && out->blue_sBIT < out->sample_depth)
      out->blue >>= out->sample_depth - out->blue_sBIT;

    out->greene += 1. / (2 * ((1U << out->green_sBIT) - 1));
    out->bluee += 1. / (2 * ((1U << out->blue_sBIT) - 1));
  }
  else
  {
    out->blue = out->green = out->red;
    out->bluef = out->greenf = out->redf;
    out->bluee = out->greene = out->rede;
  }

  if ((out->colour_type & PNG_COLOR_MASK_ALPHA) || out->colour_type == PNG_COLOR_TYPE_PALETTE)
  {
    out->alpha = sample_scale (out->alphaf, scale);
    out->alphae += 1. / (2 * ((1U << out->alpha_sBIT) - 1));
  }
  else
  {
    out->alpha = scale; /* opaque */
    out->alphaf = 1;    /* Override this. */
    out->alphae = 0;    /* It's exact ;-) */
  }

  if (sig_bits && out->alpha_sBIT < out->sample_depth)
    out->alpha >>= out->sample_depth - out->alpha_sBIT;
}
 ...
 * ones.
 */
static void transform_display_init (transform_display* dp, png_modifier* pm, png_uint_32 id,
                                    const image_transform* transform_list)
{
  memset (dp, 0, sizeof *dp);

  /* Standard fields */
  standard_display_init (dp, pm, id, do_read_interlace, pm->use_update_info);
...
static void transform_info_imp (transform_display* dp, png_struct* pp, png_infop pi)
{
  ...
  standard_info_part2 (dp, pp, pi, 1 /*images*/);
  ...
  case PNG_COLOR_TYPE_GRAY:
    if (dp->output_bit_depth == 1 || dp->output_bit_depth == 2 || dp->output_bit_depth == 4)
      break;
    /* FALLTHROUGH */
  default:
  ...
  error: {
    ...
    pos = safecat (message, sizeof message, 0, "invalid final bit depth: colour type(");
   ...
   */
  {
    ...
    test_pixel.colour_type = dp->colour_type; /* input */
    test_pixel.bit_depth = dp->bit_depth;
     ...
     */
    test_pixel.have_tRNS = dp->is_transparent != 0;
    test_pixel.red_sBIT = test_pixel.green_sBIT = test_pixel.blue_sBIT = test_pixel.alpha_sBIT =
      test_pixel.sample_depth;
    ...
    {
      char message[128];
      size_t pos = safecat (message, sizeof message, 0, "internal: sample depth ");
...
static void transform_range_check (const png_struct* pp, unsigned int r, unsigned int g,
                                   ...
                                   double in, unsigned int out, png_byte sample_depth, double err,
                                   double limit, const char* name, double digitization_error)
{
...
static void transform_image_validate (transform_display* dp, const png_struct* pp, png_infop pi)
{
  /* Constants for the loop below: */
  const png_store* const ps = dp->ps;
  png_byte in_ct = dp->colour_type;
  png_byte in_bd = dp->bit_depth;
  png_uint_32 w = dp->w;
  png_uint_32 h = dp->h;
  png_byte out_ct = dp->output_colour_type;
  png_byte out_bd = dp->output_bit_depth;
  png_byte sample_depth = (png_byte)(out_ct == PNG_COLOR_TYPE_PALETTE ? 8 : out_bd);
  png_byte red_sBIT = dp->red_sBIT;
  png_byte green_sBIT = dp->green_sBIT;
  png_byte blue_sBIT = dp->blue_sBIT;
  png_byte alpha_sBIT = dp->alpha_sBIT;
  int have_tRNS = dp->is_transparent;
  ...
  else
  {
    ...
    if (sample_depth != 16 || in_sample_depth > 8 || !dp->pm->calculations_use_input_precision)
      digitization_error = .5;
  ...
  for (y = 0; y < h; ++y)
  {
    ...
    for (x = 0; x < w; ++x)
    {
      ...
      image_pixel_init (&in_pixel, std, in_ct, in_bd, x, dp->palette, NULL);
       ...
       */
      image_pixel_init (&out_pixel, pRow, out_ct, out_bd, x, out_palette, &in_pixel);
                            ...
                            const image_transform* transform_listIn, const char* const name)
{
  ...
  try
  {
    ...
    pp = set_modifier_for_read (d.pm, &pi, d.id, full_name);
    standard_palette_init (&d);
    ...
    if (d.pm->progressive)
    {
      ...
      png_set_progressive_read_fn (pp, &d, transform_info, progressive_row, transform_end);
  ...
  catch (png_modifier* fault)
  {
    modifier_reset (fault);
  }
}
...
/* To save code: */
extern void image_transform_default_ini (const image_transform* in,
                                         transform_display* out); /* silence GCC warnings */

void /* private, but almost always needed */
image_transform_default_ini (const image_transform* in, transform_display* out)
{
...
static void image_transform_png_set_palette_to_rgb_mod (const image_transform* in,
                                                        image_pixel* out, const png_struct* pp,
                                                        const transform_display* display)
{
  if (out->colour_type == PNG_COLOR_TYPE_PALETTE)
    image_pixel_convert_PLTE (out);

  in->next->mod (in->next, out, pp, display);
}

static int image_transform_png_set_palette_to_rgb_add (image_transform* in,
                                                       const image_transform** out,
                                                       png_byte colour_type, png_byte bit_depth)
{
  UNUSED (bit_depth)
                                                       ...
                                                       png_infop pi)
{
   ...
   */
  if (out->has_tRNS)
    out->is_transparent = 1;

  in->next->set (in->next, out, pp, pi);
}

static void image_transform_png_set_tRNS_to_alpha_mod (const image_transform* in,
                                                       image_pixel* out, const png_struct* pp,
                                                       const transform_display* display)
{
#if PNG_LIBPNG_VER < 10700
...
#if PNG_LIBPNG_VER >= 10700
    if (out->colour_type != PNG_COLOR_TYPE_PALETTE
        && (out->colour_type & PNG_COLOR_MASK_ALPHA) == 0)
#endif
      image_pixel_add_alpha (out, display, 0 /*!for background*/);
...
static int image_transform_png_set_tRNS_to_alpha_add (image_transform* in,
                                                      const image_transform** out,
                                                      png_byte colour_type, png_byte)
{
  in->next = *out;
  *out = in;
                                                     ...
                                                     png_infop pi)
{
  ...
  /* NOTE: in doesn't result in tRNS expansion. */
  in->next->set (in->next, out, pp, pi);
}

static void image_transform_png_set_gray_to_rgb_mod (const image_transform* in, image_pixel* out,
                                                     const png_struct* pp,
                                                     const transform_display* display)
{
   ...
   */
  if ((out->colour_type & PNG_COLOR_MASK_COLOR) == 0 && out->have_tRNS)
    image_pixel_add_alpha (out, display, 0 /*!for background*/);

  /* Simply expand the bit depth and alter the colour type as required. */
  if (out->colour_type == PNG_COLOR_TYPE_GRAY)
  {
    /* RGB images have a bit depth at least equal to '8' */
    if (out->bit_depth < 8)
      out->sample_depth = out->bit_depth = 8;
     ...
     */
    out->colour_type = PNG_COLOR_TYPE_RGB;
  }

  else if (out->colour_type == PNG_COLOR_TYPE_GRAY_ALPHA)
    out->colour_type = PNG_COLOR_TYPE_RGB_ALPHA;

  in->next->mod (in->next, out, pp, display);
}

static int image_transform_png_set_gray_to_rgb_add (image_transform* in,
                                                    const image_transform** out,
                                                    png_byte colour_type, png_byte bit_depth)
{
  UNUSED (bit_depth)
...
static void image_transform_png_set_expand_mod (const image_transform* in, image_pixel* out,
                                                const png_struct* pp,
                                                const transform_display* display)
{
  ...
  if (out->colour_type == PNG_COLOR_TYPE_PALETTE)
    image_pixel_convert_PLTE (out);
  else if (out->bit_depth < 8) /* grayscale */
    out->sample_depth = out->bit_depth = 8;
...
static int image_transform_png_set_expand_add (image_transform* in, const image_transform** out,
                                               png_byte colour_type, png_byte bit_depth)
{
  UNUSED (bit_depth)
 ...
 */
static void image_transform_png_set_expand_gray_1_2_4_to_8_set (const image_transform* in,
                                                                transform_display* out,
                                                                png_struct* pp, png_infop pi)
{
...
static void image_transform_png_set_expand_gray_1_2_4_to_8_mod (const image_transform* in,
                                                                image_pixel* out,
                                                                const png_struct* pp,
                                                                const transform_display* display)
{
#if PNG_LIBPNG_VER < 10700
  image_transform_png_set_expand_mod (in, out, pp, display);
#else
  /* Only expand grayscale of bit depth less than 8: */
  if (out->colour_type == PNG_COLOR_TYPE_GRAY && out->bit_depth < 8)
    out->sample_depth = out->bit_depth = 8;
...
static int image_transform_png_set_expand_gray_1_2_4_to_8_add (image_transform* in,
                                                               const image_transform** out,
                                                               png_byte colour_type,
                                                               png_byte bit_depth)
{
#if PNG_LIBPNG_VER < 10700
  return image_transform_png_set_expand_add (in, out, colour_type, bit_depth);
#else
  UNUSED (bit_depth)
...
static void image_transform_png_set_expand_16_mod (const image_transform* in, image_pixel* out,
                                                   const png_struct* pp,
                                                   const transform_display* display)
{
  ...
  if (out->bit_depth < 16)
    out->sample_depth = out->bit_depth = 16;

  in->next->mod (in->next, out, pp, display);
}

static int image_transform_png_set_expand_16_add (image_transform* in,
                                                  const image_transform** out,
                                                  png_byte colour_type, png_byte bit_depth)
{
  UNUSED (colour_type)
                                                  ...
                                                  png_infop pi)
{
  ...
  out->max_gamma_8 = PNG_MAX_GAMMA_8;
#endif
  in->next->set (in->next, out, pp, pi);
}

static void image_transform_png_set_scale_16_mod (const image_transform* in, image_pixel* out,
                                                  const png_struct* pp,
                                                  const transform_display* display)
{
  if (out->bit_depth == 16)
...
static int image_transform_png_set_scale_16_add (image_transform* in,
                                                 const image_transform** out, png_byte colour_type,
                                                 png_byte bit_depth)
{
  UNUSED (colour_type)
...
/* png_set_strip_16 */
static void image_transform_png_set_strip_16_set (const image_transform* in,
                                                  transform_display* out, png_struct* pp,
                                                  png_infop pi)
{
  png_set_strip_16 (pp);
...
static void image_transform_png_set_strip_16_mod (const image_transform* in, image_pixel* out,
                                                  const png_struct* pp,
                                                  const transform_display* display)
{
  if (out->bit_depth == 16)
...
static int image_transform_png_set_strip_16_add (image_transform* in,
                                                 const image_transform** out, png_byte colour_type,
                                                 png_byte bit_depth)
{
  UNUSED (colour_type)
...
static void image_transform_png_set_strip_alpha_mod (const image_transform* in, image_pixel* out,
                                                     const png_struct* pp,
                                                     const transform_display* display)
{
  if (out->colour_type == PNG_COLOR_TYPE_GRAY_ALPHA)
    out->colour_type = PNG_COLOR_TYPE_GRAY;
  else if (out->colour_type == PNG_COLOR_TYPE_RGB_ALPHA)
    out->colour_type = PNG_COLOR_TYPE_RGB;
...
static int image_transform_png_set_strip_alpha_add (image_transform* in,
                                                    const image_transform** out,
                                                    png_byte colour_type, png_byte bit_depth)
{
  UNUSED (bit_depth)
...
static void image_transform_png_set_rgb_to_gray_ini (const image_transform* in,
                                                     transform_display* out)
{
  png_modifier* pm = out->pm;
  ...
  {
    if (out->bit_depth == 16 || pm->assume_16_bit_calculations)
    {
      ...
      if (out->bit_depth < 16)
        out->max_gamma_8 = PNG_MAX_GAMMA_8;
#endif
      out->pm->limit += pow ((out->bit_depth == 16 || out->max_gamma_8 > 14
                                 ? 8.
                                 : 6. + (1 << (15 - out->max_gamma_8)))
                                / 65535,
                              data.gamma);
    }
    ...
    else
    {
       ...
       */
#if DIGITIZE
      out->pm->limit += pow (2.0 / 255, data.gamma);
#else
      out->pm->limit += pow (1.0 / 255, data.gamma);
#endif
    }
  }
...
static void image_transform_png_set_rgb_to_gray_set (const image_transform* in,
                                                     transform_display* out, png_struct* pp,
                                                     png_infop pi)
{
...
#define API_cvt(x)   ((double)(x) / PNG_FP_1)
    ...
    if ((API_function (pp, pi, &rX, &rY, &rZ, &gX, &gY, &gZ, &bX, &bY, &bZ) & PNG_INFO_cHRM) != 0)
    {
      double maxe;
      const char* el;
      color_encoding e, o;
       ...
       */
      modifier_current_encoding (out->pm, &o);
...
static void image_transform_png_set_rgb_to_gray_mod (const image_transform* in, image_pixel* out,
                                                     const png_struct* pp,
                                                     const transform_display* display)
{
  if ((out->colour_type & PNG_COLOR_MASK_COLOR) != 0)
  {
    double gray, err;
    ...
    {
      png_modifier* pm = display->pm;
      unsigned int sample_depth = out->sample_depth;
      unsigned int calc_depth = (pm->assume_16_bit_calculations ? 16 : sample_depth);
      unsigned int gamma_depth =
        (sample_depth == 16
           ? display->max_gamma_8
           : (pm->assume_16_bit_calculations ? display->max_gamma_8 : sample_depth));
      int isgray;
      double r, g, b;
...
#define DU(v, d, r) (digitize (v * (1 + DBL_EPSILON), d, r) * (1 + DBL_EPSILON))
       ...
       */
      gray = r * data.red_coefficient + g * data.green_coefficient + b * data.blue_coefficient;

      {
        int do_round = data.gamma != 1 || calc_depth == 16;
        const double ce = 2. / 32768;

        graylo = DD (rlo * (data.red_coefficient - ce) + glo * (data.green_coefficient - ce)
                       + blo * (data.blue_coefficient - ce),
                     calc_depth, do_round);
        if (graylo > gray) /* always accept the right answer */
          graylo = gray;

        grayhi = DU (rhi * (data.red_coefficient + ce) + ghi * (data.green_coefficient + ce)
...
#undef DU
       ...
       */
      if (isgray)
        err = (out->rede + out->greene + out->bluee) / 3;
...
#else /* !DIGITIZE */
    {
      ...
      if (r == g && r == b)
      {
        ...
        if (data.gamma == 1)
        {
           ...
           */
          gray = r * data.red_coefficient + g * data.green_coefficient + b * data.blue_coefficient;
          err = re * data.red_coefficient + ge * data.green_coefficient + be * data.blue_coefficient
                + 2. / 32768 + gray * 5 * DBL_EPSILON;
        }
        ...
        else
        {
           ...
           */
          png_modifier* pm = display->pm;
          double in_qe = (out->sample_depth > 8 ? .5 / 65535 : .5 / 255);
          double out_qe =
            (out->sample_depth > 8
               ? .5 / 65535
               : (pm->assume_16_bit_calculations ? .5 / (1 << display->max_gamma_8) : .5 / 255));
    ...
    out->bluef = out->greenf = out->redf = gray;
    out->bluee = out->greene = out->rede = err;

    /* The sBIT is the minimum of the three colour channel sBITs. */
    if (out->red_sBIT > out->green_sBIT)
      out->red_sBIT = out->green_sBIT;
    if (out->red_sBIT > out->blue_sBIT)
      out->red_sBIT = out->blue_sBIT;
    out->blue_sBIT = out->green_sBIT = out->red_sBIT;

    /* And remove the colour bit in the type: */
    if (out->colour_type == PNG_COLOR_TYPE_RGB)
      out->colour_type = PNG_COLOR_TYPE_GRAY;
    else if (out->colour_type == PNG_COLOR_TYPE_RGB_ALPHA)
      out->colour_type = PNG_COLOR_TYPE_GRAY_ALPHA;
  }

  in->next->mod (in->next, out, pp, display);
}

static int image_transform_png_set_rgb_to_gray_add (image_transform* in,
                                                    const image_transform** out,
                                                    png_byte colour_type, png_byte bit_depth)
{
  UNUSED (bit_depth)
...
static void image_transform_png_set_background_set (const image_transform* in,
                                                    transform_display* out, png_struct* pp,
                                                    png_infop pi)
{
  ...
  image_pixel_init (&data, random_bytes, colour_type, bit_depth, 0 /*x*/, 0 /*unused: palette*/,
                    NULL /*format*/);
...
static void image_transform_png_set_background_mod (const image_transform* in, image_pixel* out,
                                                    const png_struct* pp,
                                                    const transform_display* display)
{
  /* Check for tRNS first: */
  if (out->have_tRNS && out->colour_type != PNG_COLOR_TYPE_PALETTE)
    image_pixel_add_alpha (out, display, 1 /*for background*/);
  ...
  if (out->alphaf < 1)
  {
    ...
    {
      out->redf = data.redf;
      out->greenf = data.greenf;
      out->bluef = data.bluef;

      out->rede = data.rede;
      out->greene = data.greene;
      out->bluee = data.bluee;

      out->red_sBIT = data.red_sBIT;
      out->green_sBIT = data.green_sBIT;
      out->blue_sBIT = data.blue_sBIT;
    }

    else /* 0 < alpha < 1 */
    {
      double alf = 1 - out->alphaf;

      out->redf = out->redf * out->alphaf + data.redf * alf;
      out->rede = out->rede * out->alphaf + data.rede * alf + DBL_EPSILON;
      out->greenf = out->greenf * out->alphaf + data.greenf * alf;
      out->greene = out->greene * out->alphaf + data.greene * alf + DBL_EPSILON;
      out->bluef = out->bluef * out->alphaf + data.bluef * alf;
      out->bluee = out->bluee * out->alphaf + data.bluee * alf + DBL_EPSILON;
    }
  ...
  if (out->colour_type == PNG_COLOR_TYPE_RGB_ALPHA)
    out->colour_type = PNG_COLOR_TYPE_RGB;
  else if (out->colour_type == PNG_COLOR_TYPE_GRAY_ALPHA)
    out->colour_type = PNG_COLOR_TYPE_GRAY;
 ...
 */
static void image_transform_png_set_invert_alpha_set (const image_transform* in,
                                                      transform_display* out, png_struct* pp,
                                                      png_infop pi)
{
...
static void image_transform_png_set_invert_alpha_mod (const image_transform* in,
                                                      image_pixel* out, const png_struct* pp,
                                                      const transform_display* display)
{
  if (out->colour_type & 4)
    out->alpha_inverted = 1;

  in->next->mod (in->next, out, pp, display);
}

static int image_transform_png_set_invert_alpha_add (image_transform* in,
                                                     const image_transform** out,
                                                     png_byte colour_type, png_byte bit_depth)
{
  UNUSED (bit_depth)
 ...
 */
static void image_transform_png_set_bgr_set (const image_transform* in, transform_display* out,
                                             png_struct* pp, png_infop pi)
{
  png_set_bgr (pp);
  in->next->set (in->next, out, pp, pi);
...
static void image_transform_png_set_bgr_mod (const image_transform* in, image_pixel* out,
                                             const png_struct* pp, const transform_display* display)
{
  if (out->colour_type == PNG_COLOR_TYPE_RGB || out->colour_type == PNG_COLOR_TYPE_RGBA)
    out->swap_rgb = 1;

  in->next->mod (in->next, out, pp, display);
}

static int image_transform_png_set_bgr_add (image_transform* in, const image_transform** out,
                                            png_byte colour_type, png_byte bit_depth)
{
  UNUSED (bit_depth)
  ...
  *out = in;
  ...
  return colour_type == PNG_COLOR_TYPE_RGB || colour_type == PNG_COLOR_TYPE_RGBA;
}
 ...
 */
static void image_transform_png_set_swap_alpha_set (const image_transform* in,
                                                    transform_display* out, png_struct* pp,
                                                    png_infop pi)
{
...
static void image_transform_png_set_swap_alpha_mod (const image_transform* in, image_pixel* out,
                                                    const png_struct* pp,
                                                    const transform_display* display)
{
  if (out->colour_type == PNG_COLOR_TYPE_GA || out->colour_type == PNG_COLOR_TYPE_RGBA)
    out->alpha_first = 1;

  in->next->mod (in->next, out, pp, display);
}

static int image_transform_png_set_swap_alpha_add (image_transform* in,
                                                   const image_transform** out,
                                                   png_byte colour_type, png_byte bit_depth)
{
  UNUSED (bit_depth)
 ...
 */
static void image_transform_png_set_swap_set (const image_transform* in, transform_display* out,
                                              png_struct* pp, png_infop pi)
{
  png_set_swap (pp);
  in->next->set (in->next, out, pp, pi);
...
static void image_transform_png_set_swap_mod (const image_transform* in, image_pixel* out,
                                              const png_struct* pp,
                                              const transform_display* display)
{
  if (out->bit_depth == 16)
...
static int image_transform_png_set_swap_add (image_transform* in, const image_transform** out,
                                             png_byte colour_type, png_byte bit_depth)
{
  UNUSED (colour_type)
...
static void image_transform_png_set_filler_set (const image_transform* in,
                                                transform_display* out, png_struct* pp,
                                                png_infop pi)
{
...
static void image_transform_png_set_filler_mod (const image_transform* in, image_pixel* out,
                                                const png_struct* pp,
                                                const transform_display* display)
{
  if (out->bit_depth >= 8
      && (out->colour_type == PNG_COLOR_TYPE_RGB || out->colour_type == PNG_COLOR_TYPE_GRAY))
  {
    unsigned int max = (1U << out->bit_depth) - 1;
    out->alpha = data.filler & max;
    out->alphaf = ((double)out->alpha) / max;
    out->alphae = 0;
     ...
     */
    out->colour_type |= 4; /* alpha added */
    out->alpha_first = data.flags == PNG_FILLER_BEFORE;
  }

  in->next->mod (in->next, out, pp, display);
}

static int image_transform_png_set_filler_add (image_transform* in, const image_transform** out,
                                               png_byte colour_type, png_byte bit_depth)
{
  in->next = *out;
  *out = in;
  ...
  return bit_depth >= 8
         && (colour_type == PNG_COLOR_TYPE_RGB || colour_type == PNG_COLOR_TYPE_GRAY);
}
...
static void image_transform_png_set_add_alpha_set (const image_transform* in,
                                                   transform_display* out, png_struct* pp,
                                                   png_infop pi)
{
  /* Need a random choice for 'before' and 'after' as well as for the
...
static void image_transform_png_set_add_alpha_mod (const image_transform* in, image_pixel* out,
                                                   const png_struct* pp,
                                                   const transform_display* display)
{
  if (out->bit_depth >= 8
...
static int image_transform_png_set_add_alpha_add (image_transform* in,
                                                  const image_transform** out,
                                                  png_byte colour_type, png_byte bit_depth)
{
  in->next = *out;
 ...
 * per pixel.
 */
static void image_transform_png_set_packing_set (const image_transform* in,
                                                 transform_display* out, png_struct* pp,
                                                 png_infop pi)
{
  png_set_packing (pp);
...
static void image_transform_png_set_packing_mod (const image_transform* in, image_pixel* out,
                                                 const png_struct* pp,
                                                 const transform_display* display)
{
  /* The general expand case depends on what the colour type is,
   ...
   */
  if (out->bit_depth < 8) /* grayscale or palette */
    out->bit_depth = 8;

  in->next->mod (in->next, out, pp, display);
}

static int image_transform_png_set_packing_add (image_transform* in, const image_transform** out,
                                                png_byte colour_type, png_byte bit_depth)
{
  UNUSED (colour_type)
 ...
 */
static void image_transform_png_set_packswap_set (const image_transform* in,
                                                  transform_display* out, png_struct* pp,
                                                  png_infop pi)
{
  png_set_packswap (pp);
...
static void image_transform_png_set_packswap_mod (const image_transform* in, image_pixel* out,
                                                  const png_struct* pp,
                                                  const transform_display* display)
{
  if (out->bit_depth < 8)
...
static int image_transform_png_set_packswap_add (image_transform* in,
                                                 const image_transform** out, png_byte colour_type,
                                                 png_byte bit_depth)
{
  UNUSED (colour_type)
 ...
 */
static void image_transform_png_set_invert_mono_set (const image_transform* in,
                                                     transform_display* out, png_struct* pp,
                                                     png_infop pi)
{
...
static void image_transform_png_set_invert_mono_mod (const image_transform* in, image_pixel* out,
                                                     const png_struct* pp,
                                                     const transform_display* display)
{
  if (out->colour_type & 4)
...
static int image_transform_png_set_invert_mono_add (image_transform* in,
                                                    const image_transform** out,
                                                    png_byte colour_type, png_byte bit_depth)
{
  UNUSED (bit_depth)
...
static void image_transform_png_set_shift_set (const image_transform* in, transform_display* out,
                                               png_struct* pp, png_infop pi)
{
   ...
   */
  unsigned int depth = out->bit_depth;
...
static void image_transform_png_set_shift_mod (const image_transform* in, image_pixel* out,
                                               const png_struct* pp,
                                               const transform_display* display)
{
  ...
  if (out->colour_type != PNG_COLOR_TYPE_PALETTE)
  {
     ...
     * does.
     */
    if (out->colour_type & 2) /* RGB channels */
    {
      out->red_sBIT = data.red;
      out->green_sBIT = data.green;
      out->blue_sBIT = data.blue;
    }

    else /* One grey channel */
      out->red_sBIT = out->green_sBIT = out->blue_sBIT = data.gray;

    out->alpha_sBIT = data.alpha;
  }

  in->next->mod (in->next, out, pp, display);
}

static int image_transform_png_set_shift_add (image_transform* in, const image_transform** out,
                                              png_byte colour_type, png_byte bit_depth)
{
  UNUSED (bit_depth)
...
static void image_transform_png_set_
  @_set (const image_transform* in, transform_display* out, png_struct* pp, png_infop pi)
{
  png_set_ @(pp);
  in->next->set (in->next, out, pp, pi);
}

static void image_transform_png_set_ @_mod (const image_transform* in, image_pixel* out,
                                            const png_struct* pp, const transform_display* display)
{
  in->next->mod (in->next, out, pp, display);
}

static int image_transform_png_set_ @_add (image_transform* in, const image_transform** out,
                                           png_byte colour_type, png_byte bit_depth)
{
  in->next = *out;
  *out = in;
...
static png_uint_32 image_transform_add (const image_transform** in, unsigned int max,
                                        ...
                                        size_t* pos, png_byte colour_type, png_byte bit_depth)
{
  for (;;) /* until we manage to add something */
  {
    ...
    *in = &image_transform_end;
    ...
    while (list != &image_transform_end)
    {
      if ((counter & mask) != 0 && list->enable && (max == 0 || list->local_use < max))
      {
        /* Candidate to add: */
        if (list->add (list, in, colour_type, bit_depth) || max == 0)
        {
        ...
        {
          /* Not useful and max>0, so remove it from *in: */
          *in = list->next;
          list->next = 0;
...
static void perform_transform_test (png_modifier* pm)
{
  ...
  while (next_format (&colour_type, &bit_depth, &palette_number, pm->test_lbg, pm->test_tRNS))
  {
    ...
    for (;;)
    {
       ...
       * command line.
       */
      counter = image_transform_add (&list, 1 /*max*/, counter, name, sizeof name, &pos,
      ...
      do
      {
        pm->repeat = 0;
        transform_test (
          pm, FILEID (colour_type, bit_depth, palette_number, pm->interlace_type, 0, 0, 0), list,
          name);
 ...
 * ones.
 */
struct gamma_display : public standard_display
{
  /* Parameters */
  png_modifier* pm;
  double file_gamma;
...
static void gamma_display_init (gamma_display* dp, png_modifier* pm, png_uint_32 id,
...
static void gamma_info_imp (gamma_display* dp, png_struct* pp, png_infop pi)
{
  /* Reuse the standard stuff as appropriate. */
  standard_info_part1 (dp, pp, pi);
...
static void init_validate_info (validate_info* vi, gamma_display* dp, const png_struct* pp,
                                int in_depth, int out_depth)
{
  ...
  if ((dp->colour_type & PNG_COLOR_MASK_ALPHA) != 0
      || (dp->colour_type == 3 && dp->is_transparent)
      || ((dp->colour_type == 0 || dp->colour_type == 2) && dp->has_tRNS))
  {
...
static double gamma_component_validate (const char* name, const validate_info* vi, unsigned int id,
                                        unsigned int od,
                                        ...
                                        const double background /* component background value */)
{
   ...
   */
  {
    ...
    if (alpha >= 0)
    {
      ...
      tcompose = 0;
      input_sample =
        gamma_component_compose (do_background, input_sample, alpha, background, &tcompose);
...
#define SBIT_ERROR 1.
        ...
        tmp = gamma_component_compose (do_background, tmp, alpha, background, NULL);
       ...
       */
      {
        ...
        if (pass == 0) /* The error condition */
        {
          ...
          store_verbose (vi->dp->pm, vi->pp, pass, msg);
      }
    }
  }
...
static void gamma_image_validate (gamma_display* dp, const png_struct* pp, png_infop pi)
{
  ...
  int processing;
  png_uint_32 y;
  const store_palette_entry* in_palette = dp->palette;
  int in_is_transparent = dp->is_transparent;
  int process_tRNS;
  int out_npalette = -1;
  ...
  processing = (vi.gamma_correction > 0 && !dp->threshold_test) || in_bd != out_bd
               || in_ct != out_ct || vi.do_background;
  process_tRNS = dp->has_tRNS && vi.do_background;
  ...
  for (y = 0; y < h; ++y)
  {
    ...
    if (processing)
    {
      ...
      for (x = 0; x < w; ++x)
      {
        ...
        unsigned int in_index = in_ct == 3 ? sample (std, 3, in_bd, x, 0, 0, 0) : 256;
        unsigned int out_index = out_ct == 3 ? sample (std, 3, out_bd, x, 0, 0, 0) : 256;

        /* Handle input alpha - png_set_background will cause the output
         * alpha to disappear so there is nothing to check.
         */
        if ((in_ct & PNG_COLOR_MASK_ALPHA) != 0 || (in_ct == 3 && in_is_transparent))
        {
          unsigned int input_alpha = in_ct == 3
                                       ? dp->palette[in_index].alpha
          ...
          if (out_ct == 3)
          {
            ...
            output_alpha = sample (pRow, out_ct, out_bd, x, samples_per_pixel, 0, 0);

          if (output_alpha != 65536)
            alpha = gamma_component_validate ("alpha", &vi, input_alpha, output_alpha, -1 /*alpha*/,
                                              0 /*background*/);
        ...
        else if (process_tRNS)
        {
          ...
          {
          case 0: /* gray */
            if (sample (std, in_ct, in_bd, x, 0, 0, 0) == dp->transparent.red)
              alpha = 0;
            break;

          case 2: /* RGB */
            if (sample (std, in_ct, in_bd, x, 0, 0, 0) == dp->transparent.red
                && sample (std, in_ct, in_bd, x, 1, 0, 0) == dp->transparent.green
                && sample (std, in_ct, in_bd, x, 2, 0, 0) == dp->transparent.blue)
              alpha = 0;
            break;
        ...
        if ((in_ct & PNG_COLOR_MASK_COLOR) == 0) /* grayscale */
          (void)gamma_component_validate ("gray", &vi, sample (std, in_ct, in_bd, x, 0, 0, 0),
        ...
        else /* RGB or palette */
        {
          (void)gamma_component_validate (
            "red", &vi,
            in_ct == 3 ? in_palette[in_index].red : sample (std, in_ct, in_bd, x, 0, 0, 0),
            out_ct == 3 ? out_palette[out_index].red : sample (pRow, out_ct, out_bd, x, 0, 0, 0),
            alpha /*component*/, vi.background_red);

          (void)gamma_component_validate (
            "green", &vi,
            in_ct == 3 ? in_palette[in_index].green : sample (std, in_ct, in_bd, x, 1, 0, 0),
            out_ct == 3 ? out_palette[out_index].green : sample (pRow, out_ct, out_bd, x, 1, 0, 0),
            alpha /*component*/, vi.background_green);

          (void)gamma_component_validate (
            "blue", &vi,
            in_ct == 3 ? in_palette[in_index].blue : sample (std, in_ct, in_bd, x, 2, 0, 0),
            out_ct == 3 ? out_palette[out_index].blue : sample (pRow, out_ct, out_bd, x, 2, 0, 0),
...
static void gamma_test (png_modifier* pmIn, png_byte colour_typeIn, png_byte bit_depthIn,
                        int palette_numberIn, int interlace_typeIn, const double file_gammaIn,
                        const double screen_gammaIn, png_byte sbitIn, int threshold_testIn,
                        const char* name, int use_input_precisionIn, int scale16In, int expand16In,
                        int do_backgroundIn, const png_color_16* bkgd_colorIn, double bkgd_gammaIn)
{
  gamma_display d;

  gamma_display_init (
    &d, pmIn, FILEID (colour_typeIn, bit_depthIn, palette_numberIn, interlace_typeIn, 0, 0, 0),
    file_gammaIn, screen_gammaIn, sbitIn, threshold_testIn, use_input_precisionIn, scale16In,
  ...
  try
  {
    ...
    /* Get a png_struct for reading the image. */
    pp = set_modifier_for_read (d.pm, &pi, d.id, name);
    standard_palette_init (&d);
    ...
    {
      /* Share the row function with the standard implementation. */
      png_set_progressive_read_fn (pp, &d, gamma_info, progressive_row, gamma_end);
    ...
    if (d.pm->log && !d.threshold_test && !d.speed)
      fprintf (stderr, "%d bit %s %s: max error %f (%.2g, %2g%%)\n", d.bit_depth,
               colour_types[d.colour_type], name, d.maxerrout, d.maxerrabs, 100 * d.maxerrpc);

    /* Log the summary values too. */
    if (d.colour_type == 0 || d.colour_type == 4)
    {
      switch (d.bit_depth)
      {
      case 1:
    ...
    else if (d.colour_type == 2 || d.colour_type == 6)
    {
      switch (d.bit_depth)
      {
      case 8:
...
static void gamma_threshold_test (png_modifier* pm, png_byte colour_type, png_byte bit_depth,
                                  int interlace_type, double file_gamma, double screen_gamma)
{
  ...
  (void)gamma_test (pm, colour_type, bit_depth, 0 /*palette*/, interlace_type, file_gamma,
                    screen_gamma, 0 /*sBIT*/, 1 /*threshold test*/, name, 0 /*no input precision*/,
...
static void perform_gamma_threshold_tests (png_modifier* pm)
{
   ...
   * without tRNS.
   */
  while (next_format (&colour_type, &bit_depth, &palette_number, pm->test_lbg_gamma_threshold,
                      pm->test_tRNS))
    if (palette_number < 2)
    {
      ...
      while (test_gamma >= .4)
      {
         ...
         */
        gamma_threshold_test (pm, colour_type, bit_depth, pm->interlace_type, test_gamma,
                              1 / test_gamma);
        test_gamma *= .95;
      }

      /* And a special test for sRGB */
      gamma_threshold_test (pm, colour_type, bit_depth, pm->interlace_type, .45455, 2.2);
...
static void gamma_transform_test (png_modifier* pm, png_byte colour_type, png_byte bit_depth,
                                  int palette_number, int interlace_type, const double file_gamma,
                                  const double screen_gamma, png_byte sbit, int use_input_precision,
                                  int scale16)
{
  size_t pos = 0;
  char name[64];
  ...
  gamma_test (pm, colour_type, bit_depth, palette_number, interlace_type, file_gamma, screen_gamma,
              sbit, 0, name, use_input_precision, scale16, pm->test_gamma_expand16, 0, 0, 0);
}
...
static void perform_gamma_transform_tests (png_modifier* pm)
{
  ...
  while (next_format (&colour_type, &bit_depth, &palette_number, pm->test_lbg_gamma_transform,
                      pm->test_tRNS))
  {
    unsigned int i, j;
    ...
    for (i = 0; i < pm->ngamma_tests; ++i)
    {
      for (j = 0; j < pm->ngamma_tests; ++j)
      {
        if (i != j)
        {
          gamma_transform_test (pm, colour_type, bit_depth, palette_number, pm->interlace_type,
                                1 / pm->gammas[i], pm->gammas[j], 0 /*sBIT*/,
...
static void perform_gamma_sbit_tests (png_modifier* pm)
{
  ...
  for (sbit = pm->sbitlow; sbit < (1 << READ_BDHI); ++sbit)
  {
    ...
    while (
      next_format (&colour_type, &bit_depth, &npalette, pm->test_lbg_gamma_sbit, pm->test_tRNS))
      if ((colour_type & PNG_COLOR_MASK_ALPHA) == 0
          && ((colour_type == 3 && sbit < 8) || (colour_type != 3 && sbit < bit_depth)))
      {
        unsigned int i;
        ...
        for (i = 0; i < pm->ngamma_tests; ++i)
        {
          ...
          for (j = 0; j < pm->ngamma_tests; ++j)
          {
            if (i != j)
            {
              gamma_transform_test (pm, colour_type, bit_depth, npalette, pm->interlace_type,
                                    1 / pm->gammas[i], pm->gammas[j], sbit,
...
#define SBIT_16_TO_8 16
  ...
  for (i = 0; i < pm->ngamma_tests; ++i)
  {
    for (j = 0; j < pm->ngamma_tests; ++j)
    {
      ...
      {
        gamma_transform_test (pm, 0, 16, 0, pm->interlace_type, 1 / pm->gammas[i], pm->gammas[j],
                              SBIT_16_TO_8, pm->use_input_precision_16to8, 1 /*scale16*/);

        if (fail (pm))
          return;

        gamma_transform_test (pm, 2, 16, 0, pm->interlace_type, 1 / pm->gammas[i], pm->gammas[j],
                              SBIT_16_TO_8, pm->use_input_precision_16to8, 1 /*scale16*/);

        if (fail (pm))
          return;

        gamma_transform_test (pm, 4, 16, 0, pm->interlace_type, 1 / pm->gammas[i], pm->gammas[j],
                              SBIT_16_TO_8, pm->use_input_precision_16to8, 1 /*scale16*/);

        if (fail (pm))
          return;

        gamma_transform_test (pm, 6, 16, 0, pm->interlace_type, 1 / pm->gammas[i], pm->gammas[j],
                              SBIT_16_TO_8, pm->use_input_precision_16to8, 1 /*scale16*/);
                                    ...
                                    int do_background, int expand_16)
{
  ...
  gamma_test (pm, colour_type, bit_depth, palette_number, interlace_type, file_gamma, screen_gamma,
              0 /*sBIT*/, 0, name, use_input_precision, 0 /*strip 16*/, expand_16, do_background,
              &background, bg);
}
...
static void perform_gamma_composition_tests (png_modifier* pm, int do_background, int expand_16)
{
   ...
   */
  while (next_format (&colour_type, &bit_depth, &palette_number, pm->test_lbg_gamma_composition,
                      pm->test_tRNS))
    if ((colour_type & PNG_COLOR_MASK_ALPHA) != 0
#if 0 /* TODO: FIXME */
        ...
        || (colour_type != 3 && palette_number != 0))
    {
      ...
      for (i = 0; i < pm->ngamma_tests; ++i)
      {
        for (j = 0; j < pm->ngamma_tests; ++j)
        {
          gamma_composition_test (pm, colour_type, bit_depth, palette_number, pm->interlace_type,
                                  1 / pm->gammas[i], pm->gammas[j], pm->use_input_precision,
...
static void summarize_gamma_errors (png_modifier* pm, png_const_charp who, int low_bit_depth,
                                    int indexed)
{
  fflush (stderr);
...
static void perform_gamma_test (png_modifier* pm, int summary)
{
  ...
  if (pm->test_gamma_background)
  {
    ...
    perform_gamma_composition_tests (pm, PNG_BACKGROUND_GAMMA_UNIQUE, pm->test_gamma_expand16);
    if (!calculations_use_input_precision)
      pm->calculations_use_input_precision = 0;
    pm->maxout8 = maxout8;
  ...
  if (pm->test_gamma_alpha_mode)
  {
    ...
    for (do_background = ALPHA_MODE_OFFSET + PNG_ALPHA_STANDARD;
         do_background <= ALPHA_MODE_OFFSET + PNG_ALPHA_BROKEN && !fail (pm); ++do_background)
      perform_gamma_composition_tests (pm, do_background, pm->test_gamma_expand16);
    if (!calculations_use_input_precision)
      pm->calculations_use_input_precision = 0;
 ...
 */
static const png_byte adam7[8][8] = {{1, 6, 4, 6, 2, 6, 4, 6}, {7, 7, 7, 7, 7, 7, 7, 7},
...
static void signal_handler (int signum)
{
  ...
  store_log (&pm, NULL /*png_struct**/, msg, 1 /*error*/);
...
int main (int argc, char** argv)
{
  ...
  while (--argc >= 1)
  {
    ...
    if (strcmp (*argv, "-v") == 0)
      ...
      summary = pm.verbose = pm.log = 0;

    else if (strcmp (*argv, "-w") == 0 || strcmp (*argv, "--strict") == 0)
      pm.treat_warnings_as_errors = 1; /* NOTE: this is the default! */

    else if (strcmp (*argv, "--nostrict") == 0)
      pm.treat_warnings_as_errors = 0;

    else if (strcmp (*argv, "--speed") == 0)
      pm.speed = 1, pm.ngamma_tests = pm.ngammas, pm.test_standard = 0, summary = 0;
    ...
    else if (strncmp (*argv, "--transform-enable=", sizeof "--transform-enable") == 0)
    {
      pm.test_transform = 1;
    ...
    else if (strncmp (*argv, "--option=", 9) == 0)
    {
      ...
      {
        fprintf (stderr, "pngvalid: %s: %s: unknown setting (use 'on' or 'off')\n", *argv, arg);
        exit (99);
      }
   ...
   * tests.
   */
  if (pm.test_standard == 0 && pm.test_size == 0 && pm.test_transform == 0 && pm.ngamma_tests == 0)
  {
  ...
  if (pm.ngamma_tests > 0 && pm.test_gamma_threshold == 0 && pm.test_gamma_transform == 0
      && pm.test_gamma_sbit == 0 && pm.test_gamma_scale16 == 0 && pm.test_gamma_background == 0
      && pm.test_gamma_alpha_mode == 0)
  {
    pm.test_gamma_threshold = 1;
    pm.test_gamma_transform = 1;
  ...
  if (summary)
  {
    printf ("%s: %s (%s point arithmetic)\n",
            (pm.nerrors || (pm.treat_warnings_as_errors && pm.nwarnings)) ? "FAIL"
                                                                          : "PASS",
            command,
  ...
  if (memstats)
  {
    printf ("Allocated memory statistics (in bytes):\n"
            ...
            (unsigned long)pm.read_memory_pool.max_max,
            (unsigned long)pm.read_memory_pool.max_limit,
            (unsigned long)pm.read_memory_pool.max_total,
            (unsigned long)pm.write_memory_pool.max_max,
            (unsigned long)pm.write_memory_pool.max_limit,
            (unsigned long)pm.write_memory_pool.max_total);
  }
   ...
   * warnings.
   */
  if (pm.nerrors || (pm.treat_warnings_as_errors && pm.nwarnings))
  {
    if (!pm.verbose)
      fprintf (stderr, "pngvalid: %s\n", pm.error);

    fprintf (stderr, "pngvalid: %d errors, %d warnings\n", pm.nerrors, pm.nwarnings);
