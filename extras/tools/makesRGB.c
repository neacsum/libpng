...
{
  unsigned int x = (unsigned int)nearbyint (finvsRGB (i));
...
int main (int argc, char** argv)
{
  ...
  for (;;)
  {
    ...
    for (i = 0; i <= 511; ++i)
    {
      ...
      calc = (unsigned int)nearbyint ((lo + adjust) * 256);
      if (calc > 65535)
      {
      ...
      calc = (unsigned int)nearbyint ((hi - lo) * 32);
      if (calc > 255)
      {
    ...
    for (i16 = 0; i16 <= 65535; ++i16)
    {
      ...
      unsigned int iexact = (unsigned int)nearbyint (255 * sRGB (i));
    ...
    if (icalc != iexact)
    {
      ...
      if (err > (max_error + .001) || err < (min_error - .001))
      {
        printf ("/* 0x%08x: exact: %3d, got: %3d [tables: %08x, %08x] (%f) */\n", i, iexact, icalc,
                png_sRGB_base[i >> 15], png_sRGB_delta[i >> 15], err);
      }
    ...
    if (icalc != iexact)
    {
      ...
      if (abs (icalc - iexact) > 1)
        printf ("/* 0x%04x: exact: %3d, got: %3d [tables: %08x, %08x] (%f) */\n", i16, iexact,
                icalc, png_sRGB_base[i >> 15], png_sRGB_delta[i >> 15], err);
    }
  }
  ...
  printf ("/* error: %g - %g, %u (%g%%) of readings inexact */\n", min_error, max_error,
          error_count, (100. * error_count) / max_input);
  printf ("/* 16-bit error: %g - %g, %u (%g%%) of readings inexact */\n", min_error16, max_error16,
          error_count16, (100. * error_count16) / 65535);
