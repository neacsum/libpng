...
{
  static TCHAR szFilter[] = TEXT ("PNG Files (*.PNG)\0*.png\0") TEXT ("All Files (*.*)\0*.*\0\0");
...
BOOL PngSaveImage (char* pstrFileName, png_byte* pDiData, int iWidth, int iHeight,
                   png_color bkgColor)
{
  ...
  png_ptr = png_create_write_struct (PNG_LIBPNG_VER_STRING, NULL, (png_error_ptr)png_cexcept_error,
                                     (png_error_ptr)NULL);
  if (!png_ptr)
  {
    fclose (pfFile);
  ...
  try
  {
    ...
    png_set_IHDR (png_ptr, info_ptr, iWidth, iHeight, ciBitDepth, PNG_COLOR_TYPE_RGB,
